...
- **Eine Steuer auf Zeile 1 wird ABGEWIESEN; ab Zeile 2 ist sie erlaubt und wird geprüft.** Der v3-Endpunkt verwirft eine Steuer nur auf der ersten (Anker-)Zeile still — lesend belegt am Produktivbestand (2026-09-25): von 101 Sammelbuchungen 2026 trägt keine einzige Zeile-1 einen Code, alle 42 Codezeilen sitzen auf Position 2+. Also die steuerfreie Zahlungs-/Gegenzeile (1050/2000/1030 …) zuerst und die besteuerte Zeile ab Position 2; eine besteuerte Zeile trägt `tax_id` und `tax_account_id` (das Konto der Zeile). Die Positionsregel ist abgeleitet, nicht per Schreibtest bewiesen — darum lesen `create`/`edit` den Beleg nach jedem Schreibzugriff zurück und vergleichen jeden gesendeten `tax_id` mit der zurückgelesenen Zeile; fehlt oder ändert sich einer, bricht der Befehl laut ab (Exit ≠ 0), nennt Beleg-ID und Zeile und verweist auf `edit`/`delete`. (Früher, 2026-08-06/07, wurde jede Zeilensteuer abgewiesen — die pauschale Lesart «v3 verwirft alle Zeilensteuern» war falsch.)
