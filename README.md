...
- **A tax on line 1 is REFUSED; on line 2+ it is allowed and verified.** The v3 endpoint silently discards a tax on the first (anchor) line only — read-verified against the live account on 2026-09-25: of 101 Sammelbuchungen in 2026 not one position-1 line carries a code, and all 42 coded lines sit at position 2+. So put a tax-free payment/counter line (1050/2000/1030 …) first and the taxed line at position 2 or later; a taxed line carries both `tax_id` and `tax_account_id` (the account of the line). The position rule is inferred, not proven by a write test, so `create`/`edit` read the document back after every write and compare each sent `tax_id` to the line that came back; a dropped or changed one aborts loudly (exit ≠ 0), names the entry id and line, and points at `edit`/`delete`. (Earlier, 2026-08-06/07, every line tax was refused — the blanket "v3 drops all line taxes" reading was wrong.)
